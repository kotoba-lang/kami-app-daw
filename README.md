...
Project-authoritative plugin slots use one manifest for processor identity, parameter labels, defaults, bounds and UI. Every manifest parameter also owns ordered, bounded musical-tick automation. The built-in saturator and compressor run as `AudioWorkletProcessor` instances; realtime playback, the master, individual stems and the aligned stem bundle schedule the same ordered processors, base values and automation ramps. Third-party plugin discovery/SDK, signed distribution and processor isolation remain explicit commercial-product gaps.
