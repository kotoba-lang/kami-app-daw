...
Project-authoritative plugin slots use one manifest for processor identity, parameter labels, defaults, bounds and UI. The project vector is the chain-order authority and the editor supports bounded up/down movement, bypass and removal. Every manifest parameter also owns ordered, bounded musical-tick automation. The built-in saturator and compressor run as `AudioWorkletProcessor` instances; realtime playback, the master, individual stems and the aligned stem bundle instantiate the same chain order and schedule the same base values and automation ramps. Third-party plugin discovery/SDK, signed distribution and processor isolation remain explicit commercial-product gaps.
