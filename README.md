...
Project-authoritative plugin slots use one manifest for processor identity, parameter labels, defaults, bounds and UI. Each insert owns bounded musical-tick wet/dry automation with project-authoritative `:linear` or `:step` interpolation. A monotonic transport clock advances the visible musical tick during playback; Read, Touch, Latch, Write and Trim modes decide whether gestures, continuing transport samples or relative offsets change automation, with repeated writes on the same tick coalesced. Tolerance-driven thinning preserves linear endpoints and deviations above the chosen error, while step thinning removes only redundant consecutive values. Project-authoritative Web MIDI mappings bind a unique channel/CC pair to plugin wet mix; explicit mapping or MIDI Learn normalizes CC input from 0–127 into the same automation-mode path, then sends the bounded value back to every connected output for motor fader/LED feedback. Standard Mackie Control messages address the first eight project tracks: 14-bit pitch-bend faders set gain, note buttons toggle solo/mute, and the DAW returns motor-fader or LED state to every output. Solo is honored by the live audio scheduler. MIDI realtime Start resets and starts the audio/tick transport, Continue restarts its clock at the displayed tick, and Stop halts it; UI or MIDI changes emit the corresponding `FA`, `FB` or `FC` byte to every output. Continue schedules decoded clips and every implemented automation lane from the selected tick. Device handlers are rebound on state changes and detached on unload. Third-party plugin isolation, Mackie bank switching, pan/record-arm and certified hardware profiles remain explicit commercial-product gaps.
