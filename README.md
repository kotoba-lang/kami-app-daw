...
Project-authoritative plugin slots use one manifest for processor identity, parameter labels, defaults, bounds and UI. Each insert owns bounded musical-tick wet/dry automation with project-authoritative `:linear` or `:step` interpolation. Armed wet-mix gestures write at the current transport tick and coalesce repeated writes on the same tick. Linear curves use complementary ramps; step curves schedule complementary wet/dry values at identical times without interpolation. Realtime playback, master and stems instantiate identical routing, order and curve semantics. Third-party plugin discovery/SDK, signed distribution and processor isolation remain explicit commercial-product gaps.
