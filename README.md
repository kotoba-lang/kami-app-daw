...
Project-authoritative plugin slots use one manifest for processor identity, parameter labels, defaults, bounds and UI. Each insert owns bounded musical-tick wet/dry automation with project-authoritative `:linear` or `:step` interpolation. A monotonic transport clock advances the visible musical tick during playback; Read, Touch, Latch and Write modes decide whether wet-mix gestures and continuing transport samples become automation, with repeated writes on the same tick coalesced. Linear curves use complementary ramps; step curves schedule complementary wet/dry values at identical times without interpolation. Realtime playback, master and stems instantiate identical routing, order and curve semantics. Third-party plugin discovery/SDK, signed distribution, processor isolation and trim automation remain explicit commercial-product gaps.
