...
Maturity: **逍遥** — project-authoritative bus gain and delay-send automation, gated K-weighted master loudness analysis, 4× oversampled true-peak measurement, ceiling-safe target normalization, direct pointer and keyboard clip movement, input monitoring, loop/comp recording, portable verified packages, recursive relinking, persistent undo/redo, waveform extraction, trim/fades, track gain automation, effects, named bus routing, delay sends, metering, master and per-track WAV export share one render path. Bus and send automation use sorted timeline points and are scheduled identically by realtime AudioContext and OfflineAudioContext renders. Normalization gain is constrained by target LUFS and the dBTP ceiling. Persistent directory grants, streaming packages above 512 MiB, plugin hosting, effect-parameter automation, certified broadcast loudness conformance and bundled multichannel stems remain follow-up scope.
