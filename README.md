...
Project-authoritative plugin slots use one manifest for processor identity, parameter labels, defaults, bounds and UI. Each insert owns bounded musical-tick wet/dry automation: the wet gain follows the mix curve while the parallel dry gain schedules its exact `1 - mix` complement at the same times. The project vector remains chain-order authority with movement, bypass and removal. Realtime playback, master and stems instantiate identical routing, order, parameter ramps and complementary mix ramps. Third-party plugin discovery/SDK, signed distribution and processor isolation remain explicit commercial-product gaps.
